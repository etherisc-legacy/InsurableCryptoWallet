...
contract MultiSigSafe {
    ...
    function execute(uint8[] sigV, bytes32[] sigR, bytes32[] sigS, uint destinationNumber, uint256 ethvalue, uint tokenNumber, uint256 tokenvalue ) public {
        ...
        require(tokenNumber < 3);                                       // check tokenNumber within limits
        ...
        // SENDING Tx
        if (tokenvalue == 0 && ethvalue > 0) {                          // verify tokenvalue == 0 and ethvalue > 0 for ETH Tx
            destination.transfer(ethvalue);                             // send ETH Tx, throws if not successfull
        } 

        if (tokenvalue > 0 && ethvalue == 0) {                          // verify tokenvalue > 0 and ethvalue == 0 for Token Tx
            TokenTransfer tok = TokenTransfer(tokenNumber);
            tok.transfer(destination, tokenvalue);                      // send Token Tx,
        }
